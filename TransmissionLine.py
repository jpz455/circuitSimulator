...
class TransmissionLine:
    ...
    def calculate_series_impedance(self):
        ...
        self.X = 2*np.pi*self.settings.f*2*10e-7*np.log(self.geometry.Deq/self.bundle.DSL)*1609
    ...
    def calculate_admittance(self):
        ...
        self.B =1j*self.settings.f*((2*np.pi*np.power(8.85,-12))/(np.log(self.geometry.Deq/self.bundle.DSC))*1609)*self.length
        self.Y =1/self.Z
    ...
    def calculate_base_values(self):
        ...
        self.z_base = (bus1.base_kv ** 2) / self.settings.s_base
